(function($)
{
	...
	function(options,arg)
	{
        ...
        //删除details行，防止选中错误的模板
		$("tr.details", $table).remove();
		...
		function createRow(rowData)
		{
			...
			$("[command]",$newRow).click(
				function()
				{
					if(settings.onExecuteCommand)
					{
						settings.onExecuteCommand($(this).attr("command"),rowData);
					}
		...
		function dealData(result)
		{
			...
			//删除旧的数据行
			$("tr.datarow,tr.details",$table).remove();
