$(document).ready(function()
{
	...
	$("select.stockinListProvider,select.stockinListNodes").change(
		function()
		{
			loadOrdersList("进货单");
    	...
    	function(nodes)
    	{
			...
			$.each(nodes,
				function(i,n)
				{
					...
					.val(n)
					.html(n)
					.appendTo("select.stockinListNodes");
				}
			);
    	}
    );		
					...
					var p=
						{
							...
							orderType:orderType,
							provider:$("select.stockinListProvider").val(),
							node:$("select.stockinListNodes").val()
						};
					return p;
				},
