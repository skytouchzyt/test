...
namespace MvcDonata.Models
{
    ...
    public class BusinessObject
    {
        ...
        public static List<Customer> GetCustomersOrderByScore()
        {
            var db = mongodbAddress.MongoDB("donata");
            var collection = db.GetCollection<Customer>("Customers");
            var q = Query.GT("Score", 0);
            var query = (from c in collection.Find(q).AsQueryable()
                         select c
                           );
            return query.ToList();
        }

        public static Customer CalcNextCustomerScore(string vip_number)
        {
            var db = mongodbAddress.MongoDB("donata");
            var collection = db.GetCollection<Customer>("Customers");
            var q = Query.GT("VIP_Number", vip_number);
            var query = (from c in collection.Find(q).AsQueryable()
                        select c).OrderBy(c=>c.VIP_Number);
            if(query.Count()>0)
            {
                var c = query.First();
                c.Score = 0;
                //首先计算堂食的积分
                var dineorders = db.GetCollection<DineOrder>("DineOrders");
                foreach(var bar in c.DineOrders)
                {
                    var dine=(from d in dineorders.AsQueryable()
                                  where d.BarCode==bar select d).First();
                    c.Score += (int)dine.Total;
                }
                //然后计算外送的积分
                var phoneorders = db.GetCollection<PhoneOrder>("PhoneOrders");
                var pquery = from p in phoneorders.AsQueryable()
                ...
                //写入数据库
                BsonDocument bd = BsonExtensionMethods.ToBsonDocument(c);


                collection.Update(Query.EQ("Phone",c.Phone), new UpdateDocument(bd), SafeMode.True);

                return c;
            }
            throw new Exception("已经找完");
        }
