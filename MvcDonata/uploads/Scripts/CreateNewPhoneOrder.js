...
function()
{
	...
	function addUpdateSuit3($target)
	{
		...
		.click(function()
		{
			...
			if(!isArrayFull(argsSalad.selections,newdish))
			{
				alert("请选择一款沙拉!");
				return false;
			}
	...
	function addUpdateSuit2($target)
	{
		...
		//添加小吃
		createDish(getDishByName("法式香蒜七味唐辛烤翅"))
		.unbind()
		.addClass("snack")
		.appendTo($part1);
		
		createDish(getDishByName("波斯香孜燃情烤翅"))
		.addClass("snack")
		.unbind()
		.appendTo($part1);
		...
		var argsSnack=
		{
			selections:[null],
			index:0,
			$dishes:$dialog.find("li.snack")
		};
		argsSnack.$dishes.bind('click',argsSnack,onSelected);
		...
		.click(function()
		{
			...
			if(!isArrayFull(argsSnack.selections,newdish))
			{
				alert("请选择一种烤翅!");
				return false;
			}
