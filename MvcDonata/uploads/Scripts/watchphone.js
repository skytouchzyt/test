...
function uploadCallIn(phone)
{
	if(phone&&phone.length<8)
		return;
	$.post("/phoneorder/createnewphonecall",
			{
				number:getCookie('deviceIndex'),
				phone:phone,
	...
	$("#createCall").click(function()
		{
			if(!$("#phone").val())
				return;
			uploadCallIn($("#phone").val());
			
		}
		);
	
	$("#setDeviceIndex").click(function()
	{
		var result=window.prompt("请输入电话设备号:",getCookie('deviceIndex')||1);
		if(result)
			setCookie('deviceIndex',result,365*10);
		return false;
	}
	);
